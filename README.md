...
This demo app does nothing special, in fact, most of it is boilerplate and comes from [this](https://laracasts.com/series/laravel-from-scratch-2017/episodes/17) great intro laracast on Laravel Auth and Config set up, __except__ that it pulls its configuration variables directly from your Manifold project (vs editing your .env file and keeping it up to date).
...
At the end, you'll have one API key, a project name, and thats all you have to keep secret, the rest gets pulled into your app via our package.
...
```
Selecting `manifoldco\manifold-laravel` from the vendor list.
...
MANIFOLD_PROJECT=
```
You can leave blank for now, we will get to modifying that a bit later on.

## Setup the Login in / Auth
For the purpose of the demo, we are simply making an app you can log in and out of, nothing else.

Thankfully laravel makes this really easy to do:
`php artisan make:auth`

This adds a bunch of views, routes, etc for account creation, password resets, etc etc.

Before we go and run the migrations, lets plug in a db from Manifold's marketplace, Jawsdb, as well as some other services.
...
3. Feel free to work ahead and add logdna, and jawsdb mysql to your project now, remember what you call the resources (I called mine logdna and jawsdb)

Also note you can do all of the above via our CLI as well, if thats more your thing.  You can find it [here](https://github.com/manifoldco/manifold-cli) and docs [here](https://docs.manifold.co/docs/install-77T6auwaaIQcgA4QGouO0).  You will need the CLI installed to generate an api token anyways (next step).

### Set up your API access Token

1. Using the [here](https://github.com/manifoldco/manifold-cli), login:
 ```
 manifold login
 ```
...
? Select Role:
  ...
  ▸ read-credentials
    ...
    admin
```
3. Edit you `.env` file (never to be committed to git) with:
```
...
Similar to provisioning LogDna, go and spin up a JawsDB MYSQL resource if you haven't already, as without it, at this point your app really can't turn on (we still need to run those migrations, remember?).
...
From here, we could plug in [MemCache](https://www.manifold.co/services/memcachier-cache) for caching (or [Redis](https://www.manifold.co/services/redisgreen) ) or even switch over to a different JawsDB offering ([Postgre](https://www.manifold.co/services/jawsdb-postgres)).  Heck, want to plug elastic in, or email, the list goes on and on :-)
